...
def copy_file(source_file, target_file):
    ...
    try:
        ...
        print(f"文件从 {source_file} 成功复制到 {target_file}")
    except Exception as e:
        print(f"复制文件时发生错误: {e}")
...
if __name__ == "__main__":
    ...
    # 首先先挂载NTUSER.DAT
    tmp_user = "TempUser" + username
    load_reg(tmp_user,os.path.join(os_root,"Users",username,"NTUSER.DAT"))
    print(os.path.join(os_root,"Users",username,"NTUSER.DAT") + "已挂载到注册表" + "HKEY_USERS\\" + tmp_user)
    print("如果" + os.path.join(os_root,"Users",username,"NTUSER.DAT") +"储存于移动磁盘,而造成磁盘占用,请在操作完成后,前往注册表卸载它")

    # 复制文件
    copy_file("reg.xml","reg_copy.xml")
    # 替换 替换路径 为 hku_tmp_user
    replace_text_in_file("reg_copy.xml","替换路径",tmp_user)

    # 复制文件
    copy_file("file.xml","file_copy.xml") 
    # 根据用户名替换file.xml的路径
    replace_text_in_file("file_copy.xml","替换用户名",username)
